...
Nt = 1000;
NElements = 520;
NNodes = NElements*2 + 1
[Cplot,Domain,TDomain] = SolveLaplaceTransient_GQ(1,0,NElements,Nt,'DL',0,'DL',1,'CN');
...
NElements = 52;
NNodes = NElements + 1
[Cplot,Domain,TDomain] = SolveLaplaceTransient(1,0, NNodes,Nt,'DL',0,'DL',1,'CN');
x = 0.4;
plot(TDomain,Cplot(round(x*NNodes),:),'LineWidth',1)
...
ylabel('C(x)')
xlabel('x')
legend('Numerical', 'NumericalGQ' ,'Analytical')
%{
for idx = 1 : length(Cplot(:,1))
